...
class Auth {
  ...
  __updateAccountInfo__ = (accountInfo: TAccountInfo) => {
    ...
    window[this.__storageType__].setItem(
      this.__storageKey__,
      JSON.stringify({
        token: {
          access:
            'eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJhZG1pbiIsImV4cCI6MTYzMTc3Njc2MH0.Y-KO0QgWFDZW1zqJsk1xpvgU0EK63fe8ZkayzXwDmn2i61CtEpD8sTFHpRKJxmKUqqYwDPNd-qXWiulT8k8jrQ',
          refresh: 'no_refresh'
        }
      })
    );
  ...
  login = async (condition: TAccountInfo): Promise<TLoginResult> => {
    //获取登录结果
    const result: TLoginResult = { status: 200, msg: '' };
    if (result.status === 1) {
      //登录成功
      const { token } = result;
